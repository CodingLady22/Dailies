...
function dirReduc(arr){
    ...
    // P - create opposites or use a conditional. Check if prev or next is an opposite, if it is, do not add it into new array.
    
    const directions = []

    for(let i = 0; i < arr.length; i++) {
        // if the last thing in directions is the opposite of arr[i], pop it out
        if(directions[directions.length - 1] === "NORTH" && arr[i] === "SOUTH" || directions[directions.length - 1] === "SOUTH" && arr[i] === "NORTH" || directions[directions.length - 1] === "EAST" && arr[i] === "WEST" || directions[directions.length - 1] === "WEST" && arr[i] === "EAST"){
            ...
            // push what is left which are not be opposites
